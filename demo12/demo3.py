...
def about():
    messagebox.showinfo(title="当前版本为1.0,欢迎使用", message="请联系作者！\n")
...
class editor():
    ...
    def savefile(self, event=None):
        if os.path.isfile(self.t.title()):
            ...
            sname = filedialog.asksaveasfilename(title="保存好哟", filetypes=[("保存文件", "*.txt")],
    ...
    def saveasfile(self, event=None):
        sname = filedialog.asksaveasfilename(title="保存好哟", filetypes=[("保存文件", "*.txt")], defaultextension=".txt")
        if sname:
            ofp = open(sname, "w")
            ofp.write(self.st.get(1.0, tk.END))
            ofp.flush()
            ...
            while pos_line <= end_line:
                if pos_line == end_line and pos_column + length > end_column:
                    ...
                    if where:
                        ...
                        again = messagebox.askokcancel(title="继续查询?")
                        if again:
                            pos_line = int(where1[0])
                            pos_column = int(sele_end_col)
                        else:
                            aa = messagebox.showinfo(title="放弃了我", message="你放弃了我!")
                            if aa:
                                sys.exit()
